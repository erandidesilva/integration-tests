...
echo "Creating the MSSQL RC and Service!"
#kubectl create -f $script_path/mysql_service.yaml
kubectl create -f $script_path/mssql_rc.yaml
#kubectl create -f $script_path/mysql_other_service.yaml
#kubectl create -f $script_path/mysql_other_rc.yaml

# Waiting for mssql-db to run. Current loop timer is 100*50 Sec.
for number in {1..100}
do
echo $(date)" Waiting for mssql to start!"
 if [ "Running" == "$(kubectl get po | grep mssql-db | awk '{print $3}')" ]
 then
  break
 fi
sleep 3
